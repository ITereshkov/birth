...
export const Settings = ({ userName, onCloseApp }: { userName?: string; onCloseApp: () => void }) => {
  ...
  return <div className="space-y-3">
    <Card>
      ...
      <div className="mb-2 flex gap-2">{(['expense','income'] as const).map((t)=><button key={t} onClick={()=>setTab(t)} className={`rounded-lg px-2 py-1 ${tab===t?'bg-slate-200':''}`}>{t==='expense'?'Расходы':'Доходы'}</button>)}</div>
    ...
    <Card><h3 className="font-semibold">Валюта</h3><p className="text-sm text-slate-500">Текущая: RUB</p></Card>
