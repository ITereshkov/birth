...
export const History = ({ onEdit, onAdd }: { onEdit: (id: string)=>void; onAdd: ()=>void }) => {
  ...
  const txs = base.filter((t) => (type==='all'||t.type===type) && (account==='all'||t.accountId===account) && (category==='all'||t.categoryId===category) && `${state.categories.find((c)=>c.id===t.categoryId)?.name||''} ${t.comment||''}`.toLowerCase().includes(query.toLowerCase()));
  const groups = txs.reduce<Record<string, typeof txs>>((acc,t)=>{ const d = t.date.slice(0,10); (acc[d] ||= []).push(t); return acc; }, {});
  ...
  return <div className="space-y-3">
    ...
    {txs.length===0 && <div className="rounded-2xl bg-white p-4 text-center text-sm text-slate-600">Пока нет операций. Добавь первую — и тут появится история ✅<div className="mt-2"><Button onClick={onAdd}>➕ Добавить</Button></div></div>}
    {Object.entries(groups).map(([day, arr]) => {
      const dt = new Date(day);
      const title = isToday(dt) ? 'Сегодня' : isYesterday(dt) ? 'Вчера' : format(dt, 'd MMM yyyy', { locale: ru });
      return <div key={day} className="rounded-2xl bg-white p-3"><div className="mb-2 text-sm font-semibold">{title}</div><div className="space-y-2">{arr.map((t)=>{const cat=state.categories.find((c)=>c.id===t.categoryId); const acc=state.accounts.find((a)=>a.id===t.accountId); return <button key={t.id} onClick={()=>onEdit(t.id)} className="flex w-full items-center justify-between text-left"><div className="text-sm"><div>{cat?.icon} {cat?.name}</div><div className="text-xs text-slate-500">{t.comment||'—'} · {acc?.name}</div></div><div className={t.type==='income'?'text-emerald-600':'text-rose-600'}>{t.type==='income'?'+':'-'}{formatMoney(t.amount)}</div></button>;})}</div></div>;
    })}
  </div>;
};
